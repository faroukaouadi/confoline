...
export default function ServicesPage() {
  return (
    <main className="relative isolate min-h-screen bg-gradient-to-b from-blue-950 to-blue-900 text-white">
      <section className="max-w-7xl 2xl:max-w-[90%] mx-auto px-4 py-12 sm:py-16 md:py-20">
        <div className="flex items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold">Services</h1>
            <p className="mt-2 max-w-3xl text-sm sm:text-base text-blue-200">
          ...
          <a
            href="#details"
            className="inline-block rounded-md bg-blue-400 px-4 py-2 text-sm font-medium text-blue-950 hover:bg-blue-300"
          >
            Learn more
          </a>
        </div>
...
function ServiceCard({ title, description }: CardProps) {
  return (
    <article className="rounded-xl border border-blue-700/50 bg-blue-900/30 p-5 shadow-md shadow-blue-950/40 backdrop-blur-sm">
      ...
      <p className="mt-2 text-xs sm:text-sm leading-relaxed text-blue-200">{description}</p>
    </article>
  );
}
